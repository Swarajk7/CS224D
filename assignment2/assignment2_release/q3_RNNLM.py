...
class RNNLM_Model(LanguageModel):
    ...
    def add_placeholders(self):
        ...
        self.input_placeholder = tf.placeholder(
            dtype=tf.int32, shape=[None, self.config.num_steps])
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embedding_layer"):
                L = tf.get_variable(
                    name="L", shape=(len(self.vocab), self.config.embed_size),
                    initializer=tf.contrib.layers.xavier_initializer())
                embeddings = tf.nn.embedding_lookup(
                    params=L, ids=self.input_placeholder)
                inputs = tf.split(
                    axis=1, num_or_size_splits=self.config.num_steps, value=embeddings)
    ...
    def add_projection(self, rnn_outputs):
        ...
        with tf.variable_scope("Projections"):
            U = tf.get_variable("U", shape=[self.config.hidden_size, len(
                self.vocab)], initializer=tf.contrib.layers.xavier_initializer())
            b2 = tf.get_variable(
                "b2", shape=[len(self.vocab)], initializer=tf.constant_initializer(0.0))
        # END YOUR CODE
        outputs = []
        for rnn_ouput in rnn_outputs:
            output = tf.matmul(rnn_ouput, U) + b2
    ...
    def add_loss_op(self, output):
        ...
        """
        # YOUR CODE HERE
        loss = sequence_loss(logits=tf.reshape(output, [
    ...
    def add_training_op(self, loss):
        ...
        with tf.variable_scope("Optimizer"):
            optimizer = tf.train.AdamOptimizer(
                learning_rate=self.config.lr, name="Adam")
    ...
    def __init__(self, config):
        ...
        output = tf.reshape(
            tf.concat(axis=1, values=self.outputs), [-1, len(self.vocab)])
    ...
    def add_model(self, inputs):
        ...
        for step, input_ in enumerate(inputs):
            with tf.variable_scope("RNNLayer") as scope:
                if step > 0:
                ...
                I = tf.get_variable("I", shape=[self.config.embed_size, self.config.hidden_size],
                                    initializer=tf.contrib.layers.xavier_initializer())
                b1 = tf.get_variable(
                    "b1", shape=[self.config.hidden_size], initializer=tf.constant_initializer(0.0))
                drop = tf.nn.dropout(tf.matmul(
                    state, H) + tf.matmul(input_, I) + b1, keep_prob=self.dropout_placeholder)
                state = tf.nn.sigmoid(drop)
