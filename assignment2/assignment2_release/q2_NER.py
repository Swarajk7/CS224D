...
class NERModel(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embedding_layer"):
                L = tf.get_variable(
                    name="L", shape=(len(self.wv), self.config.embed_size), initializer=tf.contrib.layers.xavier_initializer())
                windowUnConcatenated = tf.nn.embedding_lookup(
                    params=L, ids=self.input_placeholder)
                window = tf.reshape(windowUnConcatenated, shape=(-1,
                                                                 self.config.window_size * self.config.embed_size), name='window')
            # END YOUR CODE
            return window
    ...
    def add_model(self, window):
        ...
        with tf.variable_scope("Layer"):
            W = tf.get_variable(
                name="W", shape=[self.config.window_size * self.config.embed_size, self.config.hidden_size],
                initializer=xavier_weight_init())
            b1 = tf.get_variable(name="b1", shape=[
                                 self.config.hidden_size],  initializer=tf.constant_initializer(0.0))

        with tf.variable_scope("Softmax"):
            U = tf.get_variable(name="U", shape=[self.config.hidden_size, self.config.label_size],
                                initializer=xavier_weight_init())
            b2 = tf.get_variable(name="b2", shape=[
                self.config.label_size], initializer=tf.constant_initializer(0.0))

        h = tf.nn.tanh(tf.matmul(window, W) + b1)
        h_dropout = tf.nn.dropout(h, keep_prob=self.dropout_placeholder)
        output = tf.matmul(h_dropout, U) + b2
        l2_loss = tf.nn.l2_loss(W) + tf.nn.l2_loss(b1) + \
    ...
    def add_loss_op(self, y):
        ...
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            labels=self.labels_placeholder, logits=y))
    ...
    def add_training_op(self, loss):
        ...
        train_op = tf.train.AdamOptimizer(
            learning_rate=self.config.lr, name="Adam").minimize(loss)
        # END YOUR CODE
        return train_op
