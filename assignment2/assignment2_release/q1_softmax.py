...
def softmax(x):
    ...
    x = tf.subtract(x, tf.reduce_max(x, axis=1, keep_dims=True))
...
def cross_entropy_loss(y, yhat):
    ...
    """
    # YOUR CODE HERE
    out = tf.log(tf.reduce_sum(tf.multiply(tf.to_float(y), yhat), axis=1))
    out = tf.reduce_sum(out)
    mone = tf.constant(-1, dtype=tf.float32)
    out = tf.multiply(mone, out)
    # END YOUR CODE
    return out
