...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        """
        # YOUR CODE HERE
        '''with tf.variable_scope("xavier_init"):
            out = tf.get_variable(name="out", shape=shape, dtype=tf.float32,
                                  initializer=tf.contrib.layers.xavier_initializer())
        '''
        eps = np.sqrt(6 / np.sum(shape))
        out = tf.random_uniform(
            shape=shape, minval=-eps, maxval=eps, dtype=tf.float32)
        # END YOUR CODE
        return out
...
def test_initialization():
    ...
    with tf.Session() as session:
        ...
        print(xavier_mat.eval(session=session))
    assert xavier_mat.get_shape() == shape
    # END YOUR CODE
