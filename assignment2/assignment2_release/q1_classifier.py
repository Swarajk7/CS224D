...
class SoftmaxModel(Model):
    ...
    def add_placeholders(self):
        ...
        """
        # YOUR CODE HERE
        self.input_placeholder = tf.placeholder(dtype=tf.float32, shape=(
            self.config.batch_size, self.config.n_features), name="input_placeholder")
        self.labels_placeholder = tf.placeholder(dtype=tf.float32, shape=(
            self.config.batch_size, self.config.n_classes), name="labels_placeholder")
        # END YOUR CODE
    ...
    def add_training_op(self, loss):
        ...
        optimizer = tf.train.GradientDescentOptimizer(
            learning_rate=self.config.lr, name="GradientDescent")
    ...
    def add_model(self, input_data):
        ...
        with tf.name_scope("HiddenLayer"):
            W = tf.Variable(
                tf.zeros(shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32), dtype=tf.float32)
            b = tf.Variable(
                tf.zeros(shape=(self.config.n_classes), dtype=tf.float32), dtype=tf.float32)
        print(W.get_shape(), input_data.get_shape(), b.get_shape())
